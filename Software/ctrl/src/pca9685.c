...
void pca9685_PWM_reset(PCA9685_t *pca9685)
{
	...
	uint8_t all_on_h_buf[2];
    	all_on_h_buf[0] = (uint8_t)(all_on_h & 0x00FF);	// Load LSB
    	all_on_h_buf[1] = (uint8_t)(all_on_h >> 8);	// Load MSB

	uint16_t all_on_l = 0x1000;
	uint8_t all_on_l_buf[2];
    	all_on_l_buf[0] = (uint8_t)(all_on_l & 0x00FF); // Load LSB
    	all_on_l_buf[1] = (uint8_t)(all_on_l >> 8);	// Load MSB
...
void pca9685_PWM_write(PCA9685_t *pca9685, uint16_t pin, uint16_t on, uint16_t off)
{
	...
	uint8_t on_val_buf[2];
    	on_val_buf[0] = (uint8_t)(on_val & 0x00FF);  	// Load LSB
    	on_val_buf[1] = (uint8_t)(on_val >> 8);	     	// Load MSB

	uint16_t off_val = (off & 0x0FFF);
	uint8_t off_val_buf[2];
    	off_val_buf[0] = (uint8_t)(off_val & 0x00FF);	// Load LSB
    	off_val_buf[1] = (uint8_t)(off_val >> 8);	// Load MSB
